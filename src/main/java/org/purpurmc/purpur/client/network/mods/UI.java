...
import org.purpurmc.purpur.client.gui.clickgui.UIClickGuiScreen;
import org.purpurmc.purpur.client.gui.clickgui.window.UIContainer;
import org.purpurmc.purpur.client.gui.clickgui.window.UIWindow;
import org.purpurmc.purpur.client.gui.clickgui.window.UIWindow.Position;
...
public class UI extends Module {
	...
	public UI() {
		super("UI", KEY_UNBOUND, ModuleCategory.RENDER, true, "Shows stuff onscreen.",
				new SettingToggle("Modulelist", true).withDesc("Shows the network list.").withChildren(                                 // 0
						new SettingToggle("InnerLine", true).withDesc("Adds an extra line to the front of the network list."),
						new SettingToggle("OuterLine", false).withDesc("Adds an outer line to the network list."),
						new SettingToggle("Fill", true).withDesc("Adds a black fill behind the network list."),
						new SettingToggle("Watermark", true).withDesc("Adds the PurpurClient watermark to the network list.").withChildren(
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asToggle().getChild(3).asToggle().getState()) {
			...
			if (watermarkMode == 0) {
				moduleListText.add(0, PurpurClient.watermark.getText().append(Text.literal(" " + PurpurClient.VERSION).styled(s -> s.withColor(TextColor.fromRgb(0xf0f0f0)))));
			} else {
				moduleListText.add(0, Text.literal("\u00a7a> Ghost Bleach " + PurpurClient.VERSION));
			}
		}
