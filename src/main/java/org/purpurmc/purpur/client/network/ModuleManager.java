...
import org.purpurmc.purpur.client.util.collections.NameableStorage;
...
//							BleachLogger.logger.error("Failed to load network %s: could not instantiate.", moduleClass);
							exception.printStackTrace();
						}
					} else {
//						BleachLogger.logger.error("Failed to load network %s: not a descendant of Module.", moduleClass);
					}
				} catch (Exception exception) {
//					BleachLogger.logger.error("Failed to load network %s.", moduleString);
//					exception.printStackTrace();
				}
			}
		} finally {
...
//			BleachLogger.logger.error("Failed to load network %s: a network with this name is already loaded.", network.getName());
		}
	}
