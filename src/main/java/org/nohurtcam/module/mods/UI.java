...
import org.nohurtcam.gui.clickgui.UIClickGuiScreen;
import org.nohurtcam.gui.clickgui.window.UIContainer;
import org.nohurtcam.gui.clickgui.window.UIWindow;
import org.nohurtcam.gui.clickgui.window.UIWindow.Position;
...
public class UI extends Module {
	...
	public UI() {
		super("UI", KEY_UNBOUND, ModuleCategory.RENDER, true, "Shows stuff onscreen.",
				new SettingToggle("Modulelist", true).withDesc("Shows the module list.").withChildren(                                 // 0
						...
						new SettingToggle("Watermark", true).withDesc("Adds the NoHurtCam watermark to the module list.").withChildren(
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asToggle().getChild(3).asToggle().getState()) {
			...
			if (watermarkMode == 0) {
				moduleListText.add(0, NoHurtCam.watermark.getText().append(Text.literal(" " + NoHurtCam.VERSION).styled(s -> s.withColor(TextColor.fromRgb(0xf0f0f0)))));
			} else {
				moduleListText.add(0, Text.literal("\u00a7a> NoHurtCam " + NoHurtCam.VERSION));
			}
		}
