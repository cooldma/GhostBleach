...
import org.nohurtcam.util.world.DamageUtils;
import org.nohurtcam.util.world.EntityUtils;
	...
	private Text getLogText() {
		...
		if (getSetting(1).asToggle().getState() && !hasTotem) {
			for (PlayerEntity player: mc.world.getPlayers()) {
				if ((!getSetting(1).asToggle().getChild(0).asToggle().getState() && NoHurtCam.friendMang.has(player))
						|| player == mc.player) {
					continue;
				}
		...
		if (getSetting(3).asToggle().getState()) {
			...
			for (PlayerEntity player: mc.world.getPlayers()) {
				if (!EntityUtils.isOtherServerPlayer(player)
						|| (!getSetting(3).asToggle().getChild(1).asToggle().getState() && NoHurtCam.friendMang.has(player))) {
					continue;
				}
