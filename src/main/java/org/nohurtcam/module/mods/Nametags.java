...
import org.nohurtcam.util.render.WorldRenderer;
...
//					e.printStackTrace();
	...
	public List<Text> getPlayerLines(PlayerEntity player) {
		...
		if (getSetting(1).asToggle().getChild(2).asToggle().getState()) { // Name
			mainText.add(((MutableText) player.getName()).formatted(NoHurtCam.friendMang.has(player) ? Formatting.AQUA : Formatting.RED));
		}
