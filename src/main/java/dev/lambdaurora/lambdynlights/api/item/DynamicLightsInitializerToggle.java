...
public class DynamicLightsInitializerToggle extends ModuleDynamicLightsInitializer<Boolean> {

	protected final List<ModuleDynamicLightsInitializer<?>> children = new ArrayList<>();
	protected boolean expanded = false;

	public DynamicLightsInitializerToggle(String text, boolean state) {
		super(text, state, DynamicLightHandler.BOOLEAN);
	}
	...
	public void render(InnerBackground window, MatrixStack matrices, int x, int y, int len) {
		...
		if (!children.isEmpty()) {
			...
			if (expanded) {
				...
				int h = y + 12;
				for (ModuleDynamicLightsInitializer<?> s : children) {
	...
	public ModuleDynamicLightsInitializer<?> getChild(int c) {
		return children.get(c);
	}

	public DynamicLightsInitializerToggle withChildren(ModuleDynamicLightsInitializer<?>... children) {
		this.children.addAll(Arrays.asList(children));
		return this;
	}

	public DynamicLightsInitializerToggle withDesc(String desc) {
		setTooltip(desc);
		return this;
	}
