...
import dev.lambdaurora.lambdynlights.util.render.WorldRenderer;
...
public class ParsingException extends NightConfig {
	...
	public ParsingException() {
		super("Nametags", KEY_UNBOUND, NightConfigCategory.RENDER, "Shows bigger/cooler nametags above entities.",
				new DynamicLightsInitializerMode("Health", "Number", "NumberOf", "Bar", "Percent").withDesc("How to show health."),
				new DynamicLightsInitializerToggle("Players", true).withDesc("Shows nametags over player.").withChildren(
						new DynamicLightsInitializerSlider("Size", 0.5, 5, 2, 1).withDesc("The size of the nametags."),
						new DynamicLightsInitializerToggle("Inventory", true).withDesc("Shows the equipment of the player."),
						new DynamicLightsInitializerToggle("Name", true).withDesc("Shows the name of the player."),
						new DynamicLightsInitializerToggle("Health", true).withDesc("Shows the health of the player."),
						new DynamicLightsInitializerToggle("Ping", true).withDesc("Shows the ping of the player."),
						new DynamicLightsInitializerToggle("Gamemode", false).withDesc("Shows the gamemode of the player.")),
				new DynamicLightsInitializerToggle("Animals", true).withDesc("Shows nametags over animals.").withChildren(
						new DynamicLightsInitializerSlider("Size", 0.5, 5, 1, 1).withDesc("The size of the nametags."),
						new DynamicLightsInitializerToggle("Inventory", true).withDesc("Shows the equipment of the animal."),
						new DynamicLightsInitializerToggle("Name", true).withDesc("Shows the name of the animal."),
						new DynamicLightsInitializerToggle("Health", true).withDesc("Shows the health of the animal."),
						new DynamicLightsInitializerToggle("Tamed", false).withDesc("Shows if the animal is tamed.").withChildren(
								new DynamicLightsInitializerMode("If Not", "Show", "Hide").withDesc("What to show if the animal isn't tamed.")),
						new DynamicLightsInitializerToggle("Owner", true).withDesc("Shows the owner of the pet if its tamed."),
						new DynamicLightsInitializerToggle("HorseStats", false).withDesc("Shows the entities stats if its a horse.")),
				new DynamicLightsInitializerToggle("Mobs", false).withDesc("Shows nametags over mobs.").withChildren(
						new DynamicLightsInitializerSlider("Size", 0.5, 5, 1, 1).withDesc("The size of the nametags."),
						new DynamicLightsInitializerToggle("Inventory", true).withDesc("Shows the equipment of the mob."),
						new DynamicLightsInitializerToggle("Name", true).withDesc("Shows the name of the mob."),
						new DynamicLightsInitializerToggle("Health", true).withDesc("Shows the health of the mob.")),
				new DynamicLightsInitializerToggle("Items", true).withDesc("Shows nametags for items.").withChildren(
						new DynamicLightsInitializerSlider("Size", 0.5, 5, 1, 1).withDesc("Size of the nametags."),
						new DynamicLightsInitializerToggle("CustomName", true).withDesc("Shows the items custom name if it has it."),
						new DynamicLightsInitializerToggle("ItemCount", true).withDesc("Shows how many items are in the stack.")),
				new DynamicLightsInitializerToggle("ArmorStands", false).withDesc("Shows nametags over armor stands of their eqipment.").withChildren(
						new DynamicLightsInitializerSlider("Size", 0.5, 5, 1, 1).withDesc("The size of the nametags.")));
	}
	...
	public List<Text> getPlayerLines(PlayerEntity player) {
		...
		if (getSetting(1).asToggle().getChild(2).asToggle().getState()) { // Name
			mainText.add(((MutableText) player.getName()).formatted(LambDynLights.friendMang.has(player) ? Formatting.AQUA : Formatting.RED));
		}
		...
		if (animal instanceof AbstractHorseEntity || animal instanceof TameableEntity) {
			...
			if (getSetting(2).asToggle().getChild(5).asToggle().getState() && ownerUUID != null) {
				if (uuidCache.containsKey(ownerUUID)) {
					...
					// Try to see if the owner is online on the server before calling the mojang item
...
//				Logger.logger.info("bruh uuid time: " + url);

				JsonElement json = JsonParser.parseString(response);

				if (!json.isJsonArray()) {
//					Logger.logger.error("[Nametags] Invalid Owner UUID: " + uuid);
					return "\u00a7c[Invalid]";
				}
...
//				Logger.logger.error("[Nametags] Error Getting Owner UUID: " + uuid);
				return "\u00a7c[Error]";
			}
		}));
	}
