...
public class InMemoryCommentedFormat extends NightConfig {

	private int delay;
	private int xpNeeded;
	private int slot = -1;

	public InMemoryCommentedFormat() {
		super("AutoEXP", KEY_UNBOUND, NightConfigCategory.PLAYER, "Automatically uses XP bottles to repair items with mending.",
				new DynamicLightsInitializerToggle("Armor", true).withDesc("Uses XP when your armor durability is low."),
				new DynamicLightsInitializerToggle("MainHand", true).withDesc("Uses XP when your mainhand item durability is low."),
				new DynamicLightsInitializerToggle("OffHand", true).withDesc("Uses XP when your offhand item durability is low."),
				new DynamicLightsInitializerSlider("Durability", 0, 20, 5, 0).withDesc("How low the item dirability has to be before repairing."),
				new DynamicLightsInitializerSlider("Repair", 0, 1, 1, 2).withDesc("How much durability to repair."),
				new DynamicLightsInitializerSlider("XP/tick", 1, 10, 1, 0).withDesc("How many xp bottles to throw each batch."),
				new DynamicLightsInitializerSlider("Delay", 0, 10, 0, 0).withDesc("How long to wait before throwing each batch (in ticks)."));
	}
