...
import dev.lambdaurora.lambdynlights.util.ItemContentUtils;
...
public class AbstractInput extends NightConfig {
	...
	public AbstractInput() {
		super("Peek", KEY_UNBOUND, NightConfigCategory.MISC, "Shows whats inside containers.",
				new DynamicLightsInitializerToggle("Containers", true).withDesc("Shows a tooltip for containers.").withChildren(
						new DynamicLightsInitializerMode("Info", "All", "Name", "None").withDesc("How to show the old tooltip.")),
				new DynamicLightsInitializerToggle("Books", true).withDesc("Show tooltips for books."),
				new DynamicLightsInitializerToggle("Maps", true).withDesc("Show tooltips for maps.").withChildren(
						new DynamicLightsInitializerSlider("Map Size", 0.25, 1.5, 0.85, 2).withDesc("How big to make the map.")));
	}
