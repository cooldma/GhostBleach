...
public class CharacterInput extends NightConfig {

	private Vec3d cachedPos;
	private int timer = 0;

	public CharacterInput() {
		super("PacketFly", KEY_UNBOUND, NightConfigCategory.MOVEMENT, "Allows you to fly with packets.",
				new DynamicLightsInitializerMode("Mode", "Phase", "Packet").withDesc("Packetfly mode."),
				new DynamicLightsInitializerSlider("HSpeed", 0.05, 2, 0.5, 2).withDesc("The horizontal speed."),
				new DynamicLightsInitializerSlider("VSpeed", 0.05, 2, 0.5, 2).withDesc("The vertical speed."),
				new DynamicLightsInitializerSlider("Fall", 0, 40, 20, 0).withDesc("How often to fall (antikick)."),
				new DynamicLightsInitializerToggle("Packet Cancel", false).withDesc("Cancel rubberband packets clientside."));
	}
