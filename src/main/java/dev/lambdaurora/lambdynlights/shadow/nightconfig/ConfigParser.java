...
public class ConfigParser extends NightConfig {

	private Vec3d addVelocity = Vec3d.ZERO;
	private long lastTime;

	public ConfigParser() {
		super("NoSlow", KEY_UNBOUND, NightConfigCategory.MOVEMENT, "Disables Stuff From Slowing You Down.",
				new DynamicLightsInitializerToggle("Slowness", true).withDesc("Removes the slowness effect."),
				new DynamicLightsInitializerToggle("SoulSand", true).withDesc("Removes soul sand slowness."),
				new DynamicLightsInitializerToggle("SlimeBlocks", true).withDesc("Removes slimeblock slowness."),
				new DynamicLightsInitializerToggle("Webs", true).withDesc("Removes cobweb slowness."),
				new DynamicLightsInitializerToggle("Berry Bush", true).withDesc("Removes berry bush slowness."),
				new DynamicLightsInitializerToggle("Items", true).withDesc("Removes the slowness while eating items."),
				new DynamicLightsInitializerToggle("Inventory", true).withDesc("Allows you to move while in inventories.").withChildren(
						new DynamicLightsInitializerToggle("Sneaking", false).withDesc("Enables the sneak key while in inventories."),
						new DynamicLightsInitializerToggle("NCPBypass", false).withDesc("Allows you to move items around on serves with NCP."),
						new DynamicLightsInitializerToggle("Rotate", true).withDesc("Allows you to use the arrow keys to rotate.").withChildren(
								new DynamicLightsInitializerToggle("PitchLimit", true).withDesc("Prevents you from being able to do a 360 pitch spin."),
								new DynamicLightsInitializerToggle("Anti-Spinbot", true).withDesc("Adds a random amount of rotation when spinning to prevent spinbot detects."))));
	}
