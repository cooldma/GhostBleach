...
public class NullObject extends NightConfig {

	private int tickDelay = 0;

	public NullObject() {
		super("AutoArmor", KEY_UNBOUND, NightConfigCategory.PLAYER, "Automatically equips armor.",
				new DynamicLightsInitializerToggle("AntiBreak", false).withDesc("Unequips your armor when its about to break."),
				new DynamicLightsInitializerToggle("PreferElytra", false).withDesc("Equips elytras instead of chestplates when possible."),
				new DynamicLightsInitializerToggle("Delay", true).withDesc("Adds a delay between equipping armor pieces.").withChildren(
						new DynamicLightsInitializerSlider("Delay", 0, 20, 1, 0).withDesc("How many ticks between putting on armor pieces.")));
	}

	@Subscribe
	public void onTick(EventTick event) {
		if (mc.player.playerScreenHandler != mc.player.currentScreenHandler || !Queue.isEmpty("autoarmor_equip"))
			return;
