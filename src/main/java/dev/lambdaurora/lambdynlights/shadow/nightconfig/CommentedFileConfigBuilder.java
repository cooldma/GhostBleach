...
public class CommentedFileConfigBuilder extends NightConfig {

	private boolean flag;
	private int lastStep = 0;
	private Deque<Double> queue = new ArrayDeque<>();

	public CommentedFileConfigBuilder() {
		super("Step", KEY_UNBOUND, NightConfigCategory.MOVEMENT, "Allows you to Run up blocks like stairs.",
				new DynamicLightsInitializerMode("Mode", "Packet", "Vanilla", "Spider", "Jump").withDesc("Step mode."),
				new DynamicLightsInitializerSlider("Height", 0.1, 20, 2, 1).withDesc("How high to be able to step (Vanilla only)."),
				new DynamicLightsInitializerToggle("Cooldown", false).withDesc("Adds a cooldown between stepping to prevent rubberbanding.").withChildren(
						new DynamicLightsInitializerSlider("Amount", 0.01, 1, 0.1, 2).withDesc("How long the cooldown is (in seconds).")));
	}
