...
public class FileNotFoundAction extends NightConfig {

	private boolean jumping;

	public FileNotFoundAction() {
		super("Speed", KEY_UNBOUND, NightConfigCategory.MOVEMENT, "Allows you to go faster, what did you expect?",
				new DynamicLightsInitializerMode("Mode", "StrafeHop", "Strafe", "OnGround", "MiniHop", "Bhop").withDesc("Speed mode."),
				new DynamicLightsInitializerSlider("Strafe", 0.15, 0.55, 0.27, 2).withDesc("Strafe speed."),
				new DynamicLightsInitializerSlider("OnGround", 0.1, 10, 2, 1).withDesc("OnGround speed."),
				new DynamicLightsInitializerSlider("MiniHop", 0.1, 10, 2, 1).withDesc("MiniHop speed."),
				new DynamicLightsInitializerSlider("Bhop", 0.1, 10, 2, 1).withDesc("Bhop speed."),
				new DynamicLightsInitializerToggle("NoInertia", false).withDesc("Prevents you from moving forcefully."));
	}
