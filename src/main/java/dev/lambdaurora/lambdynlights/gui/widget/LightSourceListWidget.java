...
public abstract class LightSourceListWidget extends WindowScreen {
	...
	public LightSourceListWidget(Text title) {
		super(title);
	}
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		for (Window w : getWindows()) {
			if (w instanceof DynamicLightsOptionsOption) {
				((DynamicLightsOptionsOption) w).updateKeys(mouseX, mouseY, keyDown, lmDown, rmDown, lmHeld, mwScroll);
			}
		}
		...
		for (Window w : getWindows()) {
			if (w instanceof DynamicLightsOptionsOption) {
				DynamicLightsOptionsOption.Tooltip tooltip = ((DynamicLightsOptionsOption) w).getTooltip();
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (button == 0) {
			if (mouseX >= width / 2 - 50 && mouseX <= width / 2 - 2 && mouseY >= 0 && mouseY <= 12) {
				...
				tryOpen(LightSourceWidget.INSTANCE);
			} else if (mouseX >= width / 2 + 2 && mouseX <= width / 2 + 50 && mouseY >= 0 && mouseY <= 12) {
				client.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1f));
				tryOpen(UILightSourceListWidget.INSTANCE);
			} else {
				lmDown = true;
				lmHeld = true;
			}
